...
export function PortfolioManager() {
    ...
    function handlePortfolioSelected(portfolioId: string) {
    ...
    function handlePortfolioDeleted(portfolioId: string) {
        ...
        // Never allow deleting the Default portfolio.
        if (portfolio.id === "11111111-1111-1111-1111-111111111111") {
            alert("You cannot remove the default portfolio.");
            return;
        }
        ...
        const selectedPortfolio = portfolios[portfolios.length - 1];
    ...
    return (
        <Panel title="">
            <div className="flex flex-col gap-4">
                <div className="flex flex-wrap gap-2">
                    {workspace.portfolios.map((portfolio) => (
                        <button
                            key={portfolio.id}
                            onClick={() =>
                                handlePortfolioSelected(portfolio.id)
                            }
                            onDoubleClick={() =>
                                handlePortfolioDeleted(portfolio.id)
                            }
                            className={
                ...
                <PortfolioOverview
                    portfolio={workspace.portfolios.find(
                        (portfolio) =>
                            portfolio.id === workspace.portfolioId,
                    )}
